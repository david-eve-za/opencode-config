...
function Install-SearXNG {
    ...
    $scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
    $composeDir = Join-Path $scriptDir "searxng"
    $composeFile = Join-Path $composeDir "docker-compose.yml"
    $envFile = Join-Path $composeDir ".env"
    
    # Check if compose file exists
    if (-not (Test-Path $composeFile)) {
        Write-Log "docker-compose.yml not found at $composeFile" "ERROR"
        exit 1
    }
    
    # Check if .env exists, create from example if not
    if (-not (Test-Path "$composeDir\.env")) {
        if (Test-Path "$composeDir\.env.example") {
            Copy-Item "$composeDir\.env.example" "$composeDir\.env"
            Write-Log "Created .env from example" "SUCCESS"
        } else {
    ...
    Write-Log "Using port $port for SearXNG" "INFO"
    
    # Update .env with the selected port
    if (-not $DryRun) {
        (Get-Content $envFile) -replace '^SEARXNG_PORT=.*', "SEARXNG_PORT=$port" | Set-Content $envFile
    }
