...
function Install-SearXNG {
    ...
    # Create settings.yml
    $settingsFile = Join-Path $configDir "settings.yml"
    if ($DryRun) {
        Write-Log "[DRY-RUN] Would create $settingsFile" "INFO"
    } else {
        @"
...
"@ | Out-File -FilePath $settingsFile -Encoding UTF8
    ...
    # Pull image
    Write-Log "Pulling SearXNG Docker image..."
    if (-not $DryRun) {
    ...
    while ($true) {
        ...
        try {
            ...
            $port++
            if ($port -gt 8180) {
                Write-Log "Could not find free port" "ERROR"
                exit 1
            }
        }
    }
    Write-Log "Using port $port for SearXNG"
    
    # Stop existing container
    $existing = docker ps -a --filter "name=searxng" --format "{{.Names}}" 2>$null
    if ($existing) {
        Write-Log "Stopping existing SearXNG container..."
        docker stop searxng 2>$null | Out-Null
        docker rm searxng 2>$null | Out-Null
    }
    
    # Run container
    Write-Log "Starting SearXNG container on port $port..."
    if ($DryRun) {
        Write-Log "[DRY-RUN] Would run: docker run -d --name searxng --restart always -p $port:8080 -v $configDir:/etc/searxng searxng/searxng" "INFO"
    } else {
        docker run -d --name searxng --restart always -p $port:8080 -v "$configDir:/etc/searxng" searxng/searxng
    }
    
    # Health check
    Write-Log "Waiting for SearXNG to be healthy on port $port..."
    $url = "http://localhost:$port/search?q=test&format=json"
    $attempts = 0
    $maxAttempts = 30
    ...
    docker logs searxng --tail 50 2>$null
    exit 1
}
